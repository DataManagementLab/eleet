...
class WikiInfoboxes():
    def __init__(self, data_dir, cache_dir):
        ...
        self.mapping = cache_dir / "rotowire-wikipedia-mapping.txt"
        self.evidence_team = cache_dir / "wikipedia-evidences-team.jsonl"
        self.evidence_player = cache_dir / "wikipedia-evidences-player.jsonl"

    def load_player_evidence_table(self):
        return pd.read_json(self.evidence_player)

    def load_team_evidence_table(self):
        return pd.read_json(self.evidence_team)
    ...
    def compute_evidences(self):
        ...
        fields = {
            "team": ["name", "conference", "division", "founded", "arena", "location", "president", "coach"],
            "player": ["name", "height", "nationality", "birth_place", "birth_date",
                       "career_position", "team_2014", "team_2015", "team_2016"]
        }
        ...
        for rotowire_name, wikipedia_name in tqdm(names_dict.items()):
            ...
            try:
                ...
                assert n_teams > 3 or n_players > 3
            ...
            if not "name" in table or not table["name"]:
                table["name"] = rotowire_name
            result[rotowire_name] = table
        with open(self.evidence_team, "w") as f:
            pd.DataFrame({k: v for k, v in result.items() if "location" in v}).T.to_json(f)
        with open(self.evidence_player, "w") as f:
            pd.DataFrame({k: v for k, v in result.items() if "location" not in v}).T.to_json(f)
