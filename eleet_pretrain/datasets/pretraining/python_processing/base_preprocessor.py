...
from eleet_pretrain.datasets.pretraining.data_import.wikidata_utils import (
    WikidataProperties,
    shorten_uri,
)
from eleet_pretrain.datasets.pretraining.python_processing.utils import (
    rand_term,
    to_dict,
)
from eleet_pretrain.datasets.pretraining.python_processing.utils import (
...
class BasePreprocessor:
        ...
        for _ in range(augment_factor):
            ...
            while True:
                if skip_too_small_groups and len(shuffled_idx) < min_group_size:
                    ...
                    len(shuffled_idx) - max_group_size > min_group_size
                ):  # emit max size group
    ...
    def compute_queries(self, docs, rows, query_set):
        ...
        for doc, row in zip(docs, rows):
            ...
            queries = {
                shorten_uri(k): v
                for k, v in to_dict(doc["_queries"], key="_attr_uri").items()
            }
        ...
        for answer in query_def["_answers"]:
            for mention in answer["_answer_mentions"]:
                for k in answer_data:
                    answer_data[k].append(
                        answer[f"_{k}"] if f"_{k}" in answer else mention[f"_{k}"]
                    )
    ...
    def process_table(
        ...
        column_names = self.get_column_names(
            columns, num_cols=num_cols, random=random_names
        )
        column_ids = list(columns)
        if LABEL_COLUMN:
            column_names = [
                self.rng.choice(LABEL_NAMES) if random_names else LABEL_NAMES[0]
            ] + column_names
        ...
        for doc in docs:
            for evidence in doc["evidence"]:
                if "properties" not in evidence:
                    ...
                    continue
                values, num_values = self.get_column_values(
                    columns, evidence, num_cols=num_cols
                )  # TODO --> add to collected data
                ...
                if LABEL_COLUMN:
                    values = [
                        rand_term(
                            ...
                            entity_id=evidence["wikidata_id"],
                            rng=self.rng,
                        )
                        if random_names
                        else evidence["label"]
                    ...
                    value_positions = (
                        self.get_covered_positions(doc, ["id"], evidence)
                        + value_positions
                    )
                    num_values = [1] + num_values

                if any(values):
                    cells = TableCell.many(
                        column_ids, values, value_positions, num_values
                    )
                    evidences.append(
                        TableRow(
                            id=doc["_id"]["_doc"],
                            cells=cells,
                            text_id=doc["text"]["text_idx"],
                            text=doc["text"]["text"],
                            relevant_text_boundaries=(
                                doc["text"]["relevant_text_start"],
                                doc["text"]["relevant_text_end"],
                            ),
                        )
                    )
    ...
    def choose_columns(self, docs, column_options, num_cols, num_overlap, include=[]):
        ...
        covers_dict = to_dict(
            [x for doc in docs for x in doc["text"]["covers"]], "s", False, "p"
        )
        ...
        num_covered = max(
            0, min(len(covered_options), num_overlap, num_cols - len(include))
        )
        num_non_covered = max(
            0, min(len(non_covered_options), num_cols - num_covered - len(include))
        )
        columns = sorted(include)[:num_cols]
        columns += list(
            self.rng.choice(
                np.array(sorted(covered_options, key=str), dtype=object),
                num_covered,
                replace=False,
            )
        )
        if non_covered_options:
            p = np.array(
                [column_options[x] for x in sorted(non_covered_options, key=str)]
            )
            p = p / np.sum(p)
            columns += list(
                self.rng.choice(
                    np.array(sorted(non_covered_options, key=str), dtype=object),
                    num_non_covered,
                    p=p,
                    replace=False,
                )
    ...
    def get_covered_positions(self, doc, columns, evidence):
        positions_dict = {
            p: [(x["start"], x["end"]) for x in v]
            for p, v in to_dict(
                to_dict(doc["text"]["covers"], "s", False).get(evidence["wikidata_id"]),
                "p",
                False,
            ).items()
        }
    ...
    def get_column_values(self, columns, evidence, num_cols):
        properties = WikidataProperties(
            evidence["properties"], self.labels, self.aliases, self.rng
        )
        values = []
        num_values = []
        for c in columns:
    ...
    def get_column_options(self, docs, exclude=[]):
        ...
        for doc in docs:
            for evidence in doc["evidence"]:
                ...
                for p in sorted(
                    (properties.keys() & self.relevant_properties) - exclude_properties
                ):
